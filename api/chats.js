...
async function askHFText(messages, codeQuery = false) {
  const systemMsg = codeQuery ? SYSTEM_CODE : SYSTEM_PROMPT;
  const maxTokens = codeQuery ? 1500 : 800;
  ...
  if (!r2.ok) throw new Error(`HF Llama HTTP ${r2.status}`);
...
async function askHFVision(messages, imageBase64) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (caption) {
        return `Voici ce que je vois sur cette image 🖼️ : ${caption}\n\n${lastUser !== 'Décris cette image en détail' ? `_(Ta question : "${lastUser}" — je fais de mon mieux avec ce modèle de vision !)_` : ''}`;
      }
    }
  ...
  // Tentative 2 : vit-gpt2 (très léger, toujours dispo)
  try {
    const r2 = await fetch(
...
// ─── 3. GROQ (fallback texte si HF surchargé) ────────────────────────────────
async function askGroq(messages, codeQuery = false) {
...
async function askCloudflare(messages) {
  ...
  const data = await r.json();
  if (!data.success) throw new Error(data.errors?.[0]?.message || 'Cloudflare failed');
  return data.result?.response || data.result?.choices?.[0]?.message?.content || 'Pas de réponse';
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (imageBase64) {
      // VISION : HF (BLIP → vit-gpt2)
      reply = await askHFVision(messages, imageBase64);
      provider = 'hf-vision';

    } else {
      // TEXTE : HF → Groq → Cloudflare
      try {
        reply = await askHFText(messages, codeQuery);
        provider = codeQuery ? 'hf-code' : 'hf-chat';
      } catch (e) {
        console.log('HF text failed, trying Groq:', e.message);
        try {
          reply = await askGroq(messages, codeQuery);
          provider = codeQuery ? 'groq-code' : 'groq-chat';
        } catch (e2) {
          console.log('Groq failed, trying Cloudflare:', e2.message);
